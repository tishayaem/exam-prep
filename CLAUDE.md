...
A revision app for a Year 5 (age 10) child preparing for **Brighton College 11+ entry** (Year 7). The school sets its **own written papers** — not ISEB: a December **Academic Assessment Day** in Year 6 examining **Maths, English, Verbal Reasoning and Non-Verbal Reasoning**, then a January **Interview & Activities Day** (two short interviews with teaching staff). **Science is not examined at 11+** — the science content here is general school revision, not admissions prep. One open question (ask admissions): whether the maths/English papers are multiple-choice or write-in. Research lives in `materials/11plus-research/` — `interview.md` has the confirmed process; `maths.md` / `verbal-reasoning.md` / `non-verbal-reasoning.md` have the per-subject taxonomies. Built so a kid can drive it on an iPad.
